...
namespace ScreenShot
{
    ...
    *****************************************************************/
    ...
    public partial class CaptureMainForm : Form
    {
        #region Const

        private const byte SCREEN_ALPHA_ = 150;                                  //遮罩层背景Alpha值    

        private Color LINE_COLOR_CUSTOM = Color.FromArgb(255, 0, 174, 255);     //自定义截图时选区线条颜色
                ...
                m_captureState == CaptureState.CreatingRect)
            {
                ...
                using (Pen picPen = new Pen(Color.Black, 1))
                {
                    ...
                    using (Bitmap bmpSrc = new Bitmap(2 * INFO_MOVING_RANGE, 2 * INFO_MOVING_RANGE))
                    {
                        ...
                        //绘制放大后的图
                        using (Bitmap bmpMagnified = MagnifyImage(bmpSrc, 3))
                        {
                            g.DrawImage(
                                bmpMagnified,
                                picRect,
